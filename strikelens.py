...
def data():
    ...
    print("Received request for symbol:", symbol)
    ...
    if result is None:
        return jsonify({
            "error": f"Tradehull returned None for {symbol}"
    }), 500
    ...
    # Use integer rounding to avoid float precision mismatches during == comparison
    def si(v):
        try:
    ...
    # -----------------------------------
    # Trading Support/Resistance Highlights
    # -----------------------------------

    step = 50

    support_zone = oc[
        oc["Strike Price"] <= atm + step
    ].copy()
    ...
    highest_pe_oi = si(
        support_zone["PE OI"].max()
    )

    highest_pe_vol = si(
        support_zone["PE Volume"].max()
    )

    highest_ce_oi = si(
        resistance_zone["CE OI"].max()
    )

    highest_ce_vol = si(
        resistance_zone["CE Volume"].max()
    )
    highest_pe_doi = si(
        support_zone["PE Chg in OI"].max()
    )

    highest_ce_doi = si(
        resistance_zone["CE Chg in OI"].max()
    )
    # ── Build rows ────────────────────────────────────────────────────────────
    rows = []
    for _, r in oc.iterrows():
        rows.append({
            ...
            "pe_ltp":      sf(r["PE LTP"]),
            # Row classification — rounded int comparison avoids float drift
            "atm":        si(r["Strike Price"]) == si(atm),
            "support":    si(r["Strike Price"]) == si(support["Strike Price"]),
            "resistance": si(r["Strike Price"]) == si(resistance["Strike Price"]),
            # Cell highlights — exactly one row will be True per column
            "max_ce_oi":  si(r["CE OI"])     == highest_ce_oi,
            "max_pe_oi":  si(r["PE OI"])     == highest_pe_oi,
            "max_ce_vol": si(r["CE Volume"]) == highest_ce_vol,
            "max_pe_vol": si(r["PE Volume"]) == highest_pe_vol,
            

            "ce_iv": sf(r["CE IV"]),
            "ce_delta": sf(r["CE Delta"]),
            "ce_theta": sf(r["CE Theta"]),
            "ce_gamma": sf(r["CE Gamma"]),
            "ce_vega": sf(r["CE Vega"]),

            "pe_iv": sf(r["PE IV"]),
            "pe_delta": sf(r["PE Delta"]),
            "pe_theta": sf(r["PE Theta"]),
            "pe_gamma": sf(r["PE Gamma"]),
            "pe_vega": sf(r["PE Vega"]),
            "max_ce_doi": si(r["CE Chg in OI"]) == highest_ce_doi,
            "max_pe_doi": si(r["PE Chg in OI"]) == highest_pe_doi,
        })
