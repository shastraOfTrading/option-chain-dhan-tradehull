...
def compute():
    ...
    # Best CE: delta 0.35–0.55, lowest IV, then lowest |theta|
    ce_cands = [r for r in rows if 0.35 <= r["ce_d"] <= 0.55 and r["ce_ltp"] > 0]
    if not ce_cands:
        ce_cands = [r for r in rows if r["ce_d"] > 0.25 and r["ce_ltp"] > 0]
    ce_cands.sort(key=lambda x: (x["ce_iv"], abs(x["ce_th"])))
    rec_ce = ce_cands[0] if ce_cands else None

    # Best PE: delta -0.55 to -0.35, lowest IV
    pe_cands = [r for r in rows if -0.55 <= r["pe_d"] <= -0.35 and r["pe_ltp"] > 0]
    if not pe_cands:
        pe_cands = [r for r in rows if r["pe_d"] < -0.25 and r["pe_ltp"] > 0]
